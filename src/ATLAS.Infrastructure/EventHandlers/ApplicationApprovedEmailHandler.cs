    ...
    public class ApplicationApprovedEmailHandler : INotificationHandler<ApplicationApprovedEvent>
    {
        ...
        public async Task Handle(ApplicationApprovedEvent notification, CancellationToken cancellationToken)
        {
            try
            {
                ...
                {
                    _logger.LogWarning("Application {ApplicationId} not found for approval email", notification.ApplicationId);
                    return;
                }
                ...
                {
                    _logger.LogWarning("Citizen {CitizenId} not found or has no email for approval email", application.CitizenId);
                    return;
                }
                ...
                var body = await _templateRenderer.RenderAsync(KnownEmailTemplates.ApprovalNotification, model, cancellationToken);
                ...
                _logger.LogInformation(
                    "Sent approval email for application {ApplicationId} to {Email}",
